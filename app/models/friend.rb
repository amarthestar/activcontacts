...
belongs_to :user
  ...
  validates :nick_name, :format=>{:with =>/^[a-zA-Z\-' ]+$/, :message => "can only contain letters"}
  validates :nick_name, :length=>{:in=>2..30, :message => "Please enter Nick name 2-30 character"}
end
